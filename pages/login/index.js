  ...
  async function submitHandlerLogin(e) {
    ...
    if (response.data.user.user_type === 'Faculty') {
      ...
      const res = await axios.get(
        `http://localhost:5000/api/faculty?id=${response.data.user.facultyID}`
      );
      ...
      <div className="w-full text-white min-h-screen flex dual-background flex-row">
        ...
        <div
          className={`w-[30%] h-[350px] ${
            !isLogin ? 'h-[460px]' : 'h-[330px]'
          } bg-tertiaryblue-200 rounded-md items-center ml-[35%] px-5 py-5 ${
            !isLogin ? 'mt-[120px]' : 'mt-[175px]'
          } shadow-md`}
        >
          {isLogin && (
            <div>
              ...
              >
                <div className="flex rounded-md px-3 py-3 w-full justify-center">
                  ...
                  <input
                    id="email"
                    placeholder="Email"
                    className="bg-tertiaryblue-100 bg-opacity-40 border-b-2 border-y-tertiaryblue-200 px-2 rounded-r-md focus:outline-none w-[60%]"
                    value={email}
                    onChange={(e) => {
                ...
                <button
                  type="submit"
                  className="px-5 py-2 rounded-lg my-5 transition-all duration-300 border-2 border-pink-600 text-pink-600 hover:bg-pink-600 hover:text-white ease-in-out"
