...
const Index = () => {
  ...
  const [endTime, setEndTime] = useState();
    async function getEventById(id){
        const response = await axios.get(`http://localhost:5000/api/events/?id=${id}`,{},{})
        console.log(response)
        setEvent(response.data[0])
        console.log(event)
        setName(event.name)
        setDes(event.description)
        setType(event.domain)
        setVenue(event.venue)
        setBanner(event.banner)
        setDate(moment(event.date).format("YYYY-MM-DD"))
        setStartTime(moment(event.startTime).format("hh:mm:ss a"))
        setEndTime(moment(event.endTime).format("hh:mm:ss a"))
    }
    useEffect(() => {
    ...
    <div className="bg-tertiaryblue-100 text-sm flex flex-col ">
      ...
      >
        <div className="flex flex-col w-[50%] mr-20">
          ...
          {(image.length>0)?
            <img src={URL.createObjectURL(image[image.length - 1])}></img>:<img src={banner}></img>}
